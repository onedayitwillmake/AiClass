...
public class GridModel {
	...
	public void setupSquares() {
		for (int i = 0; i < _gridColumnCount; ++i) {
			for (int j = 0; j < _gridRowCount; ++j) {
				_gridSquares[i][j] = new GridSquare(i * _squareSize, j * _squareSize,(int) _squareSize, app);
	...
	public GridSquare getSquareAtGridPosition(PVector asVector) {
		return this.getSquareAtGridPosition((int)asVector.x, (int)asVector.y);
	}
